...
{
    public partial class kuaiji : Form, ITRTCCloudCallback, ITRTCLogCallback
    {
        ...
        public kuaiji(TRTCLoginForm loginForm)
        {
            // 初始化 Form
            InitializeComponent();
            // 添加自定义渲染窗口控件
            ...
            if (isHasLinkedUser)
            {
                ...
                this.noLinkedUser.Visible = true;
        ...
        public void EnterRoom()
        {
            ...
            this.roomLabel.Text = "用户ID：" + trtcParams.userId;
            this.localUserLabel.Text = "房间号：" + trtcParams.roomId;
        ...
        public void onRemoteUserEnterRoom(string userId)
        {
            if (this.IsHandleCreated)
                this.BeginInvoke(new Action(() => {
                    // 添加远端进房用户信息
                    mRemoteUsers.Add(new RemoteUserInfo() { account = userId, position = -1 });
        ...
        private int GetIdleRemoteVideoPosition(String userId)
        {
            ...
            //    return 2;
            //else
            if (string.IsNullOrEmpty(this.remoteUserLabel3.Text) || this.remoteUserLabel3.Text.Equals(userId))
                return 3;
        ...
        private void OnPKUserExit(string userId)
        {
            foreach (PKUserInfo info in mPKUsers)
            {
                if (info.account.Equals(userId) && info.isEnterRoom)
                {
        ...
        public void onUserVideoAvailable(string userId, bool available)
        {
            if (this.IsHandleCreated)
                this.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        if (user.account.Equals(userId)) return true;
                        else return false;
                    });
                    if (!isExit) return;
                    if (available)
        ...
        public void onFirstVideoFrame(string userId, TRTCVideoStreamType streamType, int width, int height)
        {
            if (this.IsHandleCreated)
                this.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        if (info.account.Equals(userId) && info.streamType == streamType)
                        {
                            info.width = width;
                            info.height = height;
                            find = true;
                        ...
                        else if (info.account.Equals(userId) && streamType != TRTCVideoStreamType.TRTCVideoStreamTypeSub)
                        {
        ...
        private void RemoveVideoMeta(string userId, TRTCVideoStreamType streamType)
        {
            foreach (UserVideoMeta info in mMixStreamVideoMeta)
            {
                if (info.account == userId && (info.streamType == streamType || 
        ...
        private void UpdateMixTranCodeInfo()
        {
            ...
            foreach (UserVideoMeta info in mMixStreamVideoMeta)
            {
                ...
                {
                    if (pKUserInfo.account.Equals(info.account))
                    {
            ...
            mixUsersArray[index].userId = localMainVideo.account;
            ...
            foreach (UserVideoMeta info in mMixStreamVideoMeta)
            {
                ...
                mixUsersArray[index].userId = info.account;
        ...
        private void SetRemoteVoiceVolume(string userId, int volume)
        {
            ...
            //else
            if (this.remoteUserLabel3.Text.Equals(userId) && this.remoteVoiceProgressBar3.Visible)
        ...
        {
            mPKUsers.Add(new PKUserInfo() { roomId = roomId, account = userId });
        }
        ...
        private void initLinkedUser() {
            this.lvlinkedUser.View = View.Details;
            this.lvlinkedUser.SmallImageList = this.avatarList;
            this.lvlinkedUser.Columns.Add("头像", 40, HorizontalAlignment.Center);
            this.lvlinkedUser.Columns.Add("昵称", 110, HorizontalAlignment.Left);
            this.lvlinkedUser.Columns.Add("连麦", 100, HorizontalAlignment.Center);

            //数据更新，UI暂时挂起，直到EndUpdate绘制控件，可以有效避免闪烁并大大提高加载速度
            this.lvlinkedUser.BeginUpdate();
            int size = this.avatarList.Images.Count;
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                lvi.SubItems.Add("吴明洲" + i);
                lvi.SubItems.Add("可连麦");
                items[i] = lvi;
            }
            this.lvlinkedUser.Items.AddRange(items);
            this.lvlinkedUser.EndUpdate();

            
            btn.Visible = false;
            btn.Text = "可以";
            btn.Click += this.button_Click;
            this.lvlinkedUser.Controls.Add(btn);

            if (this.lvlinkedUser.SelectedItems.Count > 0)
            {
                this.btn.Location = new Point(this.lvlinkedUser.SelectedItems[0].SubItems[2].Bounds.Left,
                    this.lvlinkedUser.SelectedItems[0].SubItems[2].Bounds.Top);
        ...
        private void button_Click(object sender, EventArgs e) {
            MessageBox.Show(this.lvlinkedUser.SelectedItems[0].SubItems[0].Text);
        }

        private void initList() {
            this.lvlinkedUser.View = View.Tile;
            //this.lvlinkedUser.Margin = 25;
            this.lvlinkedUser.SmallImageList = this.avatarList;
            this.lvlinkedUser.BeginUpdate();
            for (int i = 1; i <= this.avatarList.Images.Count; i++) {
                ListViewItem lvi = new ListViewItem();
                lvi.ImageIndex = i - 1;
                lvi.Text = "item" + i;
                ...
                this.lvlinkedUser.Items.Add(lvi);
            }
            this.lvlinkedUser.EndUpdate();
        }

        private void lvlinkedUser_ColumnWidthChanged(object sender, ColumnWidthChangedEventArgs e)
        {
            //ColumnHeader header = this.lvlinkedUser.Columns[e.ColumnIndex];
            //header.Width = lvlinkedUser.Columns[e.ColumnIndex].Width;
        }

        private void lvlinkedUser_SelectedIndexChanged(object sender, EventArgs e)
        {
